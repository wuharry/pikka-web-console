...
export default defineConfig(({ command }) => {
  const root = ${JSON.stringify(consoleRoot)};
  return {
    root,
    mode: 'development',
    publicDir: false,
    ...
    // 只在 build 階段帶 input，且用相對於 root 的路徑
    ...(command === 'build'
      ? {
          build: {
            outDir: 'pikka-console-dist',
    ...
    // 避免和主專案的 Vite 快取互踩
    cacheDir: 'node_modules/.vite-pikka',
  };
});
`
...
module.exports = defineConfig(({ command }) => {
  ...
  return {
    ...
    resolve: {
      alias: {
        '@': cwd,
        'pikka-web-console': path.join(cwd, 'node_modules/pikka-web-console'),
      }
    },
